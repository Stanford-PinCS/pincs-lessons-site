  ...
  const messages = state?.sentMessages.map(
    (m: {
      ...
      if (m.contents.type === "text") {
        contents = <span className="break-words">{m.contents.contents}</span>;
      } else if (m.contents.type === "image") {
        const file = getFile(m.contents.uri);
        if (!file || file.type !== "Asset") {
          contents = <span className="break-words">Could not load image</span>;
        } else {
          contents = <img src={file.url} style={{ height: 100, width: 100 }} />;
        }
      } else {
      ...
      return {
        from: m.from,
        contents,
        responseOptions: m.from === "bot" ? m.responseOptions : undefined,
      };
    }
  );
            ...
            mostRecentMessage?.responseOptions && (
              ...
              >
                {mostRecentMessage.responseOptions.map(
                  (opt: string, i: Key | null | undefined) => (
                    <ChatPill
                      text={opt}
                      onClick={() => {
                        sendResponse(opt);
